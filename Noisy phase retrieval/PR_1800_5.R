...
wirtinger_flow <- function(X, y, init = FALSE, lr = 0.002, times = 1000){
  n <- nrow(X)
  p <- ncol(X)
...
for(s in 1:length(mnum)){
  ...
  est_list <- foreach(t = 1:iters, .packages = c("MASS")) %dopar% {
    ...
    est_reboot <- wirtinger_flow(X_reboot, unlist(y_reboot), est_local[1,], lr = 0.02)
...
save(all_estimator, file = "est_all_1800_5.RData")
save(df_mse, df_bias, mnum, N, file = "est_res_1800_5.RData")
