...
wirtinger_flow <- function(X, y, beta = FALSE, lr = 1e-3, times = 10000){
  n <- nrow(X)
  if(!beta[1]){
...
for(s in 1:length(mnum)){
  ...
  est_list <- foreach(t = 1:iters, .packages = c("MASS")) %dopar% {
    res <- tryCatch({
      ...
      est_reboot <- wirtinger_flow(Z, yz, est_local[1,], lr = 1e-2)
...
save(all_estimator, file = "est_all_20.RData")
save(df_mse, df_bias, mnum, N, file = "est_res_20.RData")
