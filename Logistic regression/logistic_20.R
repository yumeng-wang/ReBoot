...
logistic_coef <- function(X, y){
  fit <- glm(y ~ X - 1, family = binomial, control = list(maxit = 200, epsilon = 1e-14))
  coef_fit <- coef(fit)
...
for(s in 1:length(mnum)){
  ...
  est_list <- foreach(i = 1:iters, .packages = c("MASS")) %dopar% {
    res <- tryCatch({
      ...
      pz <- unlist(lapply(1:m, function(j)
      ...
      #--------------return results-------------------
      list(opt=est_opt, ave=est_aver, savgm=est_savgm, reboot=est_reboot,
  ...
  mse_opt_all <- sapply(1:length(est_list), function(j) l2norm(est_opt_all[,j] - theta))
  mse_ave_all <- sapply(1:length(est_list), function(j) l2norm(est_ave_all[,j] - theta))
  mse_savgm_all <- sapply(1:length(est_list), function(j) l2norm(est_savgm_all[,j] - theta))
  mse_reboot_all <- sapply(1:length(est_list), function(j) l2norm(est_reboot_all[,j] - theta))
  mse_csl1_all <- sapply(1:length(est_list), function(j) l2norm(est_csl1_all[,j] - theta))
  mse_csl2_all <- sapply(1:length(est_list), function(j) l2norm(est_csl2_all[,j] - theta))
...
save(est_all, file="est_all_20.RData")
...
save(len_run, df_mse_mean, df_mse_sd, df_bias, mnum, N, file="est_res_20.RData", col.names=T)
