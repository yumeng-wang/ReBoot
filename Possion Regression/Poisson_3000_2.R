...
possion_coef <- function(X, y){
  fit <- glm(y ~ X - 1, family="poisson", control=list(maxit=200))
  coef_fit <- coef(fit)
...
#-----------------------------------------
d <- 2
R <- 200  # reboot sample size rate
r <- 0.3  # subsampling rate
N <- 3000
mnum <- c(150, 120, 100, 80, 60, 40)
cat("p is ", d, "\n", sep="")
...
for(s in 1:length(mnum)){
  ...
  est_list <- foreach(i = 1:iters, .packages = c("MASS")) %dopar% {
    ...
    #---------------reboot estimator----------------
    Z <- matrix(runif(NR * d), NR, d)
    lambdaz <- lapply(1:m, function(j) exp(Z[((j - 1) * nR + 1) : (j * nR),] %*% est_local[, j]))
    lambdaz <- as.vector(unlist(lambdaz))
...
save(est_all, file="est_all_3000_2.RData")
...
save(df_mse_mean, df_mse_sd, df_bias, mnum, N, file="est_res_3000_2.RData", col.names=T)
